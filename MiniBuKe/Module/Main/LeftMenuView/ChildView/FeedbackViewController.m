...
@interface FeedbackViewController ()<FeedbackCenterViewDelegate>

@property(nonatomic,strong) UIView *topView;
@property(nonatomic,strong) UIView *middleView;
@property(nonatomic,strong) UIView *bottomView;

@property(nonatomic,strong) FeedbackCenterView *mFeedbackCenterView;
...
//    [MobClick event:EVENT_FEEDBACK_11];
...
- (void)initView{
    ...
    [_middleView setBackgroundColor: COLOR_STRING(@"#EBEBEB")];
...
{
    self.mFeedbackCenterView = [[FeedbackCenterView xibView] init];
    self.mFeedbackCenterView.delegate = self;
    [self.middleView addSubview: self.mFeedbackCenterView];
    [self.mFeedbackCenterView updateView: self.view.frame.size];
}
...
#pragma FeedbackCenterViewDelegate
-(void) onSubmit {
    [self.navigationController popViewControllerAnimated:YES];
}
