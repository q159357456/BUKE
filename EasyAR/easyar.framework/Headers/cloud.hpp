...
public:
    ...
    void setMirrorCamera();
    void open(std::string server, std::string appKey, std::string appSecret, std::shared_ptr<CallbackScheduler> callbackScheduler, std::function<void(CloudStatus, std::vector<std::shared_ptr<Target>>)> callback);
    bool close();
...
static void FunctorOfVoidFromCloudStatusAndListOfPointerOfTarget_func(void * _state, easyar_CloudStatus, easyar_ListOfPointerOfTarget *, /* OUT */ easyar_String * * _exception);
...
inline void CloudRecognizer::setMirrorCamera()
{
    easyar_CloudRecognizer_setMirrorCamera(cdata_.get());
}
inline void CloudRecognizer::open(std::string arg0, std::string arg1, std::string arg2, std::shared_ptr<CallbackScheduler> arg3, std::function<void(CloudStatus, std::vector<std::shared_ptr<Target>>)> arg4)
{
    easyar_CloudRecognizer_open(cdata_.get(), std_string_to_easyar_String(arg0).get(), std_string_to_easyar_String(arg1).get(), std_string_to_easyar_String(arg2).get(), (arg3 == nullptr ? nullptr : arg3->get_cdata().get()), FunctorOfVoidFromCloudStatusAndListOfPointerOfTarget_to_c(arg4));
}
...
static void FunctorOfVoidFromCloudStatusAndListOfPointerOfTarget_func(void * _state, easyar_CloudStatus arg0, easyar_ListOfPointerOfTarget * arg1, /* OUT */ easyar_String * * _exception)
{
    *_exception = nullptr;
    try {
