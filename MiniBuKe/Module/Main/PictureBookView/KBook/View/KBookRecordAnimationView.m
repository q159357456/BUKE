...
@interface KBookRecordAnimationView()
...
@implementation KBookRecordAnimationView
...
-(void)setup
{
    ...
    self.timeLabel.frame = CGRectMake((self.frame.size.width - self.middleInterval)*0.5, (self.frame.size.height - 20)*0.5, self.middleInterval, 20);
    self.timeLabel.text = @"";
    self.timeLabel.textColor = COLOR_STRING(@"#9B9B9B");
    self.timeLabel.font = MY_FONT(14);
...
//    self.timeLabel.center = CGPointMake(self.itemWidth * 0.5f, self.itemHeight * 0.5f);
    ...
    self.lineWidth = (self.itemWidth - self.middleInterval) * 0.5 / self.numberOfItems;
}
...
-(void)updateItems
{
    ...
    int leftX = (self.itemWidth - self.middleInterval + self.lineWidth) * 0.5f;
    int rightX = (self.itemWidth + self.middleInterval - self.lineWidth) * 0.5f;
    ...
    for (int i = 0; i < self.numberOfItems / 2; i++) {
        ...
        CAShapeLayer *itemLineLeft = [self.itemLineLayers objectAtIndex:i + self.numberOfItems/2];
        itemLineLeft.path = [linePathLeft CGPath];
        ...
        CAShapeLayer *itemLineRight = [self.itemLineLayers objectAtIndex:i];
        itemLineRight.path = [linePathRight CGPath];
    }
...
-(void)start
{
    if (self.displayLink == nil) {
        ...
        self.displayLink.frameInterval = 4.f;
